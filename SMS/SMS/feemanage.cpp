#include "feemanage.h"
#include "ui_feemanage.h"
#include <QMessageBox>
#include "mainwindow.h"
#include "globle.h"
#include <QtDebug>
#include "manger.h"

feemanage::feemanage(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::feemanage)
{
    ui->setupUi(this);
}

feemanage::~feemanage()
{
    delete ui;
}

void feemanage::on_fee_querypushButton_clicked()
{
    QString id=ui->fee_IDlineEdit->text();
    //ui->fee_namelineEdit->clear();
    //ui->fee_banjilineEdit->clear();;
    QString term;
    //ui->fee_majorlineEdit->clear();
    ui->fee_feelineEdit->clear();
    ui->fee_arrearage_lineEdit->clear();
    QString all;
    if(ui->radioButton->isChecked())
    {
        term = "term 1";
    }else if(ui->radioButton_2->isChecked())
    {
        term = "term 2";
    }else if(ui->radioButton_3->isChecked())
    {
        term = "term 3";
    }else if(ui->radioButton_4->isChecked())
    {
        term = "term 4";
    }else{
        term = "";
    }

    qDebug()<<term;

    if(id.compare("")==0||term.compare("")==0)
    QMessageBox::information(this, "警告", "查询条件输入不足");

    QSqlDatabase db;
    if(QSqlDatabase::contains("qt_sql_default_connection"))
    db = QSqlDatabase::database("qt_sql_default_connection");
    else
        db=QSqlDatabase::addDatabase("QODBC");
        db.setHostName("localhost");
        db.setDatabaseName("ORCL");
        db.setUserName("eagle");
        db.setPassword("161530301");
    if (!db.open())
    qDebug() << "Failed to connect to root mysql ";
     else qDebug() << "open";


    QSqlQuery query(db);
    db.exec("SET NAMES 'GBK'");


   query.exec("select f.fee,f.arrearage,s.name,s.major,s.banji from fee f,student s where f.id='"+id+"' and f.term='"+term+"' and s.id = f.id");

    if(query.next()){
       qDebug()<<"第一条数据为"<<query.value(0).toString();
        ui->fee_feelineEdit->setText(query.value(0).toString());
        ui->fee_arrearage_lineEdit->setText(query.value(1).toString());

        //ui->fee_namelineEdit->setText(query.value(2).toString());
        QString name = query.value(2).toString();
        //ui->fee_majorlineEdit->setText(query.value(3).toString());
        QString major = query.value(3).toString();
        //ui->fee_banjilineEdit->setText(query.value(4).toString());
        QString banji = query.value(4).toString();
        all = "NAME: " + name + "    MAJOR: " + major + "    CLASS: " + banji;
    }
        else
         {
           if(id.compare("")==0||term.compare("")==0)
                     return;

           else   QMessageBox::information(this, "警告", "不存在你所要查找的数据");

         }
    ui->textEdit->setPlainText(all);
}

void feemanage::on_fee_addpushButton_clicked()
{
    QString id=ui->fee_IDlineEdit->text();
    //QString name=ui->fee_namelineEdit->text();
    //QString banji=ui->fee_banjilineEdit->text();
    //QString major=ui->fee_majorlineEdit->text();
    QString term;
    QString fee=ui->fee_feelineEdit->text();
    QString arrearage=ui->fee_arrearage_lineEdit->text();
    if(ui->radioButton->isChecked())
    {
        term = "term 1";
    }else if(ui->radioButton_2->isChecked())
    {
        term = "term 2";
    }else if(ui->radioButton_3->isChecked())
    {
        term = "term 3";
    }else if(ui->radioButton_4->isChecked())
    {
        term = "term 4";
    }

    QSqlDatabase db;
    if(QSqlDatabase::contains("qt_sql_default_connection"))
    db = QSqlDatabase::database("qt_sql_default_connection");
    else
        db=QSqlDatabase::addDatabase("QODBC");
        db.setHostName("localhost");
        db.setDatabaseName("ORCL");
        db.setUserName("eagle");
        db.setPassword("161530301");
    if (!db.open())
    qDebug() << "Failed to connect to root mysql admin";
     else qDebug() << "open";

    QSqlQuery query(db);
    db.exec("SET NAMES 'GBK'");
    query.exec("select id,term from fee");
    bool T2=true;
    while(query.next())
               {
                   QString id1= query.value(0).toString();
                   QString term1= query.value(1).toString();

                   if(id.compare(id1)==0&&term.compare(term1)==0)
                   {

                    QMessageBox::information(this ,tr("提示") , tr("该编号已存在不允许再次添加!"));
                      T2=false;
                   }
                 }
    if(T2==true)
    {
         QString sql;
         sql = QString("insert into fee ( id,term,fee,arrearage)"
                  " VALUES ('%1', '%2' , '%3', '%4' )")
            .arg(id).arg(term).arg(fee).arg(arrearage);


        bool ok = query.exec(sql);
       if(ok)
       {
        db.commit();
        QMessageBox::information(this ,tr("提示") , tr("添加成功!"));
      }
      else
      {
        QMessageBox::information(this ,tr("提示") , tr("添加失败!"));
      }
   }
}

void feemanage::on_fee_deletepushButton_clicked()
{
    QString id=ui->fee_IDlineEdit->text();
    QString term;
    QSqlDatabase db;
    if(ui->radioButton->isChecked())
    {
        term = "term 1";
    }else if(ui->radioButton_2->isChecked())
    {
        term = "term 2";
    }else if(ui->radioButton_3->isChecked())
    {
        term = "term 3";
    }else if(ui->radioButton_4->isChecked())
    {
        term = "term 4";
    }

    if(QSqlDatabase::contains("qt_sql_default_connection"))
    db = QSqlDatabase::database("qt_sql_default_connection");
    else
        db=QSqlDatabase::addDatabase("QODBC");
        db.setHostName("localhost");
        db.setDatabaseName("ORCL");
        db.setUserName("eagle");
        db.setPassword("161530301");
    if (!db.open())
        qDebug() << "Failed to connect to root mysql admin";
         else qDebug() << "open";



        QSqlQuery query(db);
        db.exec("SET NAMES 'GBK'");
        query.exec("select id,term from fee");
    bool T2=true;
    while(query.next())
               {
                   QString id1= query.value(0).toString();
                   QString term1= query.value(1).toString();
                   qDebug()<<id1<<term1;

                   if(id.compare(id1)==0&&term.compare(term1)==0)
                  {
                       QString sql;
                       sql = QString("DELETE FROM fee "
                                     " where id='"+id+"' and term='"+term+"' ");
                       QSqlQuery query;
                       bool ok = query.exec(sql);
                       if(ok)
                       {
                           db.commit();
                           QMessageBox::information(this ,tr("提示") , tr("删除成功!"));
                           T2=false;
                       }
                       else
                       {
                           QMessageBox::information(this ,tr("提示") , tr("删除失败!"));
                           T2=false;
                       }

                   }
                 }
    if(T2==true)
    {
       QMessageBox::information(this ,tr("提示") , tr("改编号不存在无法删除"));
    }
}

void feemanage::on_fee_updatepushButton_clicked()
{
    QString id=ui->fee_IDlineEdit->text();
    QString term;
    QString fee=ui->fee_feelineEdit->text();
    QString arrearage=ui->fee_arrearage_lineEdit->text();
    if(ui->radioButton->isChecked())
    {
        term = "term 1";
    }else if(ui->radioButton_2->isChecked())
    {
        term = "term 2";
    }else if(ui->radioButton_3->isChecked())
    {
        term = "term 3";
    }else if(ui->radioButton_4->isChecked())
    {
        term = "term 4";
    }

    QSqlDatabase db;
    if(QSqlDatabase::contains("qt_sql_default_connection"))
    db = QSqlDatabase::database("qt_sql_default_connection");
    else
        db=QSqlDatabase::addDatabase("QODBC");
        db.setHostName("localhost");
        db.setDatabaseName("ORCL");
        db.setUserName("eagle");
        db.setPassword("161530301");
    if (!db.open())
        qDebug() << "Failed to connect to root mysql admin";
         else qDebug() << "open";

        QSqlQuery query(db);
        db.exec("SET NAMES 'GBK'");
        query.exec("select id,term from fee");
    bool T2=true;
    while(query.next())
               {
                   QString id1= query.value(0).toString();
                   QString term1 = query.value(1).toString();
                   if(id.compare(id1)==0&&term.compare(term1)==0)
                  {
                       QString sql;
                       sql = QString("UPDATE fee "
                                     "set  fee= '%0',arrearage= '%1' "
                                     " where id='"+id+"' and term = '"+term+"' ")
                                            .arg(fee).arg(arrearage);

                       QSqlQuery query;
                       bool ok = query.exec(sql);
                       if(ok)
                       {
                           db.commit();
                           QMessageBox::information(this ,tr("提示") , tr("更改成功!"));
                           T2=false;
                       }
                       else
                       {
                           QMessageBox::information(this ,tr("提示") , tr("更改失败!"));
                           T2=false;
                       }

                   }
                 }
    if(T2==true)
    {

       QMessageBox::information(this ,tr("WARNING") , tr("ID not exit!"));


    }
}

void feemanage::on_fee_backpushButton_clicked()
{
    this->hide();
    manger *sx=new manger;
    sx->show();
}
